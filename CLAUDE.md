...
- Automated end-to-end smoke test: `npx vite build && npx electron . --smoke` — boots the **built** app against an isolated temp `userData` and asserts against real Electron + real Chromium. Covers: parse→thumbnail→protocol; tag-write round-trips (MP3/WAV/FLAC); the reindex pipeline; file-open/Services launch arbitration; re-opening the already-loaded file; and, driven through `executeJavaScript`, the tracklist UI (sort-cycle clicks, keyboard, a 20k-track bench), the column header's geometry/alignment/isolation-from-scroll, the Tag Editor window (centred on its display, the Search Online popup, dedupe-then-limit), and a guard that `.glass-panel` has a real computed `blur()` in the **built** CSS.
  ...
  - For one-off DOM inspection outside the smoke suite (e.g. verifying a narrow-width layout), a standalone script run via `electron <script>.mjs` can create a `BrowserWindow` at any size and drive it with `executeJavaScript` — same technique `smokeTest.mjs` uses, no dev toolchain needed beyond the running `vite` dev server. **Creating a second `BrowserWindow` in the same process after the first sometimes fails to load** (`ERR_FAILED`, reproducible on this machine) — run one check per `electron` invocation rather than reusing a process across multiple test windows.
...
Below `NARROW_LAYOUT_BREAKPOINT` (560px, `App.jsx`) — window `minWidth` is 380 — the whole shell restructures for phone-portrait widths: the sidebar becomes an overlay drawer, the player panel becomes a mini-player, and Now Playing becomes a single-column Song/Lyrics tab view. The tracklist/player-panel structure above that breakpoint is untouched — but a few controls are now deliberately shared across both widths rather than narrow-exclusive (the sidebar hamburger; see [Sidebar](docs/ARCHITECTURE.md#sidebar-homelibrary-collapsible)), so "narrow" and "full-width" are no longer two fully independent code paths everywhere. See [Narrow layout](docs/ARCHITECTURE.md#narrow-layout-phone-portrait-widths).

Library's tracklist has extra sortable columns (Album/Year/Genre) beyond Title/lyrics/Duration, which appear and disappear one at a time as the window narrows — a separate mechanism from the narrow-layout breakpoint above, gated by its own thresholds in `App.jsx`, none of which may drop below 780px (a hardcoded 700px smoke-test check depends on it). See [Tracklist engine → Library's extra columns](docs/ARCHITECTURE.md#tracklist-engine-tracklistjsx-trackrowjsx-tracklistheaderjsx-hooks).
...
| Give an element inside `.glass-panel` (or any `backdrop-filter` surface) a z-index to stack it above a true sibling, like the narrow-mode drawer | `backdrop-filter` creates its own stacking context, so that z-index only competes with the panel's *other* children — it can never out-rank something outside the panel, no matter how high. The element has to actually be a sibling of what it needs to beat, not just carry a bigger number. Confirmed live via `document.elementFromPoint()` — the covered element wasn't just visually dimmed, it was genuinely unclickable. [Details](docs/ARCHITECTURE.md#narrow-layout-phone-portrait-widths) |
| Dedupe against current library membership *before* deciding what a "Play Next" action should queue | Regression that shipped and went unnoticed: Finder Services' "Play Next in Sonus" (`queueTracksFromService`) and the drag-and-drop confirm modal's Play Next (`resolvePendingTracks`) both used to filter out already-in-the-library tracks before touching `forcedNextQueueRef` — the ref that actually drives what plays next (`playNext()` reads only that, not row position). A track you Play-Next'd that happened to already be in your library silently did nothing. Fix: dedupe only decides which rows get newly *inserted*; every selected track, new or not, still reaches `forcedNextQueueRef`. `add-to-queue` is unaffected — for that action, an already-present track really is a no-op. [Details](docs/ARCHITECTURE.md#finder-services-integration-add-to-queue-play-next) |
| Trust `getComputedStyle` + `document.elementFromPoint()` to prove text is rendering the color it declares | Both can say "fine" while the pixels are wrong. Real case: narrow Now Playing's title rendered ~186/255 gray with `color: rgb(255,255,255)` and a hit-test returning the text itself — the artwork card's `zIndex: 1` drop shadow (24px downward offset) was painting *over* the text, and **box-shadows paint over later siblings but are never hit-testable**, so `elementFromPoint` cannot reveal one. Forcing the text onto its own compositing layer changed nothing either (that doesn't alter z-order). The fix was `position: relative; zIndex: 2` on the text block; the diagnosis only came from `webContents.capturePage()` + PIL, sampling actual pixels against a known reference. When a color looks wrong but the CSS says it's right, go straight to pixels. [Details](docs/ARCHITECTURE.md#now-playing-glass-backdrop-contrast-guarantee) |
| Put a side effect inside a `setState(updater)` function | `src/main.jsx` wraps the app in `StrictMode`, which **calls updater functions twice** in dev — never in the packaged build. Anything order-dependent inside one breaks only under `npm run dev`, exactly where everyone looks. Real case: the tag-saved handler reloaded `<audio>` inside `setCurrentTrack(ct => …)`; the first call's `audio.load()` synchronously reset the element to 0/paused, so the second call captured "resume at 0, wasn't playing" and the song stopped at the start after every Save — while the smoke suite (built app) passed. Do the effect in the handler body, read state through the mirror refs (`currentTrackRef` etc.), and keep updaters pure. The save-while-playing smoke case guards the production path; the dev-only half is guarded only by keeping updaters pure. [Details](docs/ARCHITECTURE.md#tag-editor-a-separate-window-not-a-view) |
...
- [Narrow layout (phone-portrait widths)](docs/ARCHITECTURE.md#narrow-layout-phone-portrait-widths)
