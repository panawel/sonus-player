...
- [Narrow layout (phone-portrait widths)](#narrow-layout-phone-portrait-widths)
...
- This is also why `electron/main.js`'s `minWidth` could safely drop from `800` to `700`, and later to `380` for narrow-layout mode (see "Narrow layout" below) — the center block no longer has a hardcoded collapse point.
...
- NP overlay wrapper: `bottom: isNarrowLayout ? 0 : 124`
- Toast notification: `bottom: isNarrowLayout ? NARROW_PLAYER_PANEL_HEIGHT + 16 : 140` (`124 + 16px` gap above the panel top)

If the panel height ever changes, update both of these. See "Narrow layout" below for why the NP overlay's offset drops to `0` rather than just swapping in the mini-player's own height.
...
## Narrow layout (phone-portrait widths)

Below `NARROW_LAYOUT_BREAKPOINT` (`560`, `App.jsx`) — a strict subset of `ULTRA_COMPACT_PANEL_BREAKPOINT` (`780`), so the icon-shrink tiers above still apply unchanged in the 560–900px range — the app shell branches into an entirely different, single-column layout instead of further shrinking the desktop one. `minWidth` in `electron/main.js` is `380`, so this range is fully reachable. `isNarrowLayout = useIsNarrow(NARROW_LAYOUT_BREAKPOINT)` gates every branch below; nothing here is a CSS media query, since these are structural DOM differences, not just sizing (Home's own `.home-quick-picks-grid` responsiveness is still a plain `@media` swap — this distinction matches how `isCompactPanel`/`isUltraCompactPanel` are already JS-gated for the same reason).

**Sidebar → overlay drawer.** Same `sidebarCollapsed` state and `setSidebarCollapsed` as the persistent-column mode — only the CSS class (`.sidebar-drawer` in `index.css`) changes, making it `position: fixed` over the content with a `.sidebar-drawer-backdrop` behind it, instead of pushing content aside, and widened to `NARROW_DRAWER_WIDTH` (240px, vs. the 88px icon-only rail at full width) with horizontal icon+label rows instead of stacked icon-over-text tiles — a slim rail only reads well as a persistent column, not a drawer people expect to scan text in. The hamburger (below) replaces `.sidebar-toggle-tab` as the trigger; selecting Home/Library/Add Files also closes the drawer. **The `sidebarCollapsed` initializer checks `window.innerWidth` directly** (not just `localStorage`) — a cold launch that starts out narrow must never show the drawer open for even one frame; correcting it a frame later via the `isNarrowLayout` effect would flash it open first. Its own top padding is a plain `20px` (`sidebarCollapsed ? '20px 0 0' : '20px 12px 0'`) — just breathing room, not sized to clear anything, since nothing sits above it at the same horizontal position anymore (see next).

**Hamburger and search both live in the draggable title-bar strip itself** (the same 42px strip that holds the traffic lights, `top: 9`-ish), not in a row of their own inside the panel — an earlier version gave the hamburger a dedicated ~42–58px row directly below the strip, which cost real vertical space for one icon and, for a while, put it in the *same* row as search, where the two collided. The current layout, three siblings of `.glass-panel` (not children of it — see why below):
- **Search** (`view === 'library'` only, hidden while the drawer is open) — the exact same shared `renderSearchField({ width, height, fontSize })` pill component full-width search uses, just called with a smaller size (180×28 narrow vs. 224×28 full width) and always centered on the full window (`left: '50vw'`) rather than offset around a sidebar column that doesn't exist in narrow mode. `zIndex: 50`.
- **"Home" label** (`view === 'home'` only, same closed-drawer-only condition) — same strip position search would occupy. `zIndex: 50`.
- **Hamburger** — `position: fixed, top: 2, right: 16, zIndex: 51`, right-aligned since the traffic lights always occupy the left edge, so there's no collision risk. Unlike search/the Home label, it stays visible and interactive **regardless of drawer state**, toggling between the hamburger and a close (✕) icon — it's the drawer's own open/close control, so it can't be something the drawer is allowed to hide.

**Why these three are true siblings of `.glass-panel` and not nested inside it, with a big z-index:** `.glass-panel`'s own `backdrop-filter` gives it a separate stacking context, so a z-index on something nested inside it only ever competes with the panel's *other* children — it can never out-rank a true sibling like the drawer (`z-index: 30`), no matter how large the number. Nesting the hamburger/search there once left them genuinely covered and unclickable behind the open drawer — confirmed live via `document.elementFromPoint()` hit-testing, not just visually dimmed. The fix was moving them out to the `.app-container` level entirely, not raising a z-index. See the matching "fails silently" row in `CLAUDE.md`.

Removing the hamburger's dedicated row eliminated the ~42–58px that used to sit between the strip and the first track row — Library's `TrackListHeader` now renders directly at the top of `.glass-panel`, right after the strip.

**Player panel → mini-player.** The full desktop panel (`{!isNarrowLayout && (...)}`) is completely untouched. In its place, a much shorter bar built by `renderMiniPlayerBar(onBarClick)` (`NARROW_PLAYER_PANEL_HEIGHT = 72`) shows artwork + title/artist + **prev/play-pause/next** (30/36/30px) — no seek bar, no volume/repeat/shuffle. `onBarClick` is the one thing that differs between its two call sites: in Library/Home it opens Now Playing (`expandNowPlaying`); on narrow Now Playing's Lyrics tab (below) it switches back to the Song tab instead, since opening Now Playing makes no sense when it's already open. The Library/Home instance is **hidden entirely while Now Playing is open** (`isNarrowLayout && !isNowPlayingOpen`), because narrow Now Playing is a full dedicated screen rather than sitting above a still-docked bar — which is also why the NP overlay's `bottom` offset drops to `0` in narrow mode instead of tracking the mini-player's height.

**Now Playing → single-column tab view.** Branches to an entirely different JSX tree (not a forced 50/50 → 100/0 width transition) inside the same always-mounted NP overlay: a top row (back chevron, a Song/Lyrics tab label reusing the same `isLyricsOpen` boolean the desktop side-panel toggle already uses, and the `⋮` menu), then either the Song tab (artwork, title/artist, the seek bar, and a transport row) or the Lyrics tab (the same lyrics block, full width, plus `renderMiniPlayerBar` docked at the bottom — see above — so there's still a way to see/control playback while reading lyrics instead of the screen being lyrics-only). The transport row (shuffle/prev/play/next/repeat) is built by `renderTransportControls({ playSize, playIconSize, playMargin, controlSize, secondarySize })`, a separate local helper in `App.jsx` — it reuses the exact same handlers/state as the desktop panel (`playPrev`, `playNext`, `toggleRepeat`, `handlePlayPauseClick`/`handlePlayPausePressStart`/`handlePlayPausePressEnd`) but not its JSX, since the desktop panel's shuffle/repeat live in a separate cluster from prev/play/next (alongside volume) and that grouping must stay untouched at full width.

No new features were added anywhere in this layout — favorite/add-to-playlist/equalizer/sleep-timer/queue-view/skip-±10s all have no Sonus equivalent and are simply absent; Playlists/Folders/Artists tab navigation for the Library was likewise left out (it stays a single flat list, just restyled). An earlier version of narrow Now Playing kept a Volume control (the one reference "utility icon" with a real Sonus feature behind it) — it was later removed at the user's request, so narrow Now Playing now has no volume control at all; full-width's own volume popup is unaffected.
